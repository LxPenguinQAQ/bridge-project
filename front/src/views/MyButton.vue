...
<script>
    export default {
        ...
        methods: {
            jumpPage() {
                if (this.type == "node") {
                    this.$router.push({path:'NodeTablePage', query: {matchId: this.matchId, address: this.address}})
                } else if (this.type == "coordinator") {
                    this.$router.push({path:'CoordinatorTablePage', query: {Id: this.address}})
                }
            }
        },
    }
